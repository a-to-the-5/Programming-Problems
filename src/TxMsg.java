import java.util.*;
public class TxMsg {
	static Vector<Character> vowls;
	static{
		vowls=new Vector<Character>();
		vowls.add('a');
		vowls.add('e');
		vowls.add('i');
		vowls.add('o');
    ...
    static String enc(String w){
    	String ret = "";
    	boolean hasCons = false;
    	for(char c:w.toCharArray())if(!vowls.contains(c)){hasCons = true; break;}
    	if(!hasCons) return w;
    	if(w.length()>0 && !vowls.contains(w.charAt(0)))
    		ret +=w.charAt(0);
    	for(int i = 1; i<w.length(); i++)
    	{
    		if(!vowls.contains(w.charAt(i)) && vowls.contains(w.charAt(i-1)))
    			ret +=w.charAt(i);
    	}
    	return ret;
    }
