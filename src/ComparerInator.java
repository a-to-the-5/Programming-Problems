...
public class ComparerInator {
	static int makeProgram(int[] A, int[] B, int[] wanted){
		boolean eb = true, ea = true, g = true, s = true;
		for (int i = 0; i < wanted.length; i++) {
			if(wanted[i]!=A[i]) ea = false;
			if(wanted[i]!=A[i]) ea = false;
			if(wanted[i]!=Math.max(A[i], B[i])) g =false;
			if(wanted[i]!=Math.max(A[i], B[i])) s =false;
		}
		if(ea||eb) return 1;
		if(g||s) return 7;
		return -1;
